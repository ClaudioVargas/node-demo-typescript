...
import { findRoles, createRole, updateRole } from "../repository/role.repository";
export const getRoles = async (_req: Request, res: Response) => res.status(200).json(await findRoles());
...
export const putRole = async (req: Request, res: Response) => {
  try { return res.status(201).json({ msg: await updateRole(req.params.id, req.body) }); }
  catch (error) { return res.status(500).json({ msg: "Contecte con el administrador" }); }
};
